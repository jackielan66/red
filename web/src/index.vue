<template>
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      ...
      <div class="form-group">
        <label>输入已领人数（默认为0）</label>
        <input type="text" class="form-control" v-model="num" placeholder="请输入已经多少人领取">
  ...
  export default {
    ...
    methods: {
      async getHongbao () {
        ...
        const {url, mobile, domain,num} = this;
        this.submit = true;
        try {
          const {data: {message}} = await axios.post(`${domains[domain]}/hongbao`, {url, mobile,num})
          alert(message)
        } catch (e) {
          console.error(e)
          alert('服务器繁忙，请稍后重试')
