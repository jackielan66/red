 ...
 * @returns {Promise<*>}
 */
async function request ({mobile, url,num} = {}) {
  ...
  return (async function lottery (phone) {
    ...
    // 计算剩余第几个为最佳红包
    const number = query.lucky_number - promotion_records.length - num;
    // console.log(number,'nub 红包剩余数量')
    // console.log(promotion_records,'nub promotion_records 红包剩余数量')
    if (number <= 0) {
      ...
      if(lucky && lucky.amount){
        return `手气最佳红包已被领取\n\n手气最佳：${lucky.sns_username}\n红包金额：${lucky.amount} 元`
      } else{
        if(query.lucky_number>promotion_records.length){
            return `红包已经抢到第 ${promotion_records.length} 人了，加油！`
        }else{
            return '\'红包被人抢完\\n或\\n服务器繁忙\''
        }
      }
